				 ...
				 uint32_t ncalls)
{
	...
	for (uint32_t i = 0; i < ncalls; i++) {
		...
		if (size < 8 || off + size > bytes)
			return 2;
		const uint8_t* payload = prog + off + 8;
		size_t plen = (size_t)(size - 8);
		int rc = 0;
		switch (kind) {
		case KIND_WRITE_PHYS:
			rc = handle_write_phys(payload, plen);
			break;
		case KIND_OUTB:
			rc = handle_outb(payload, plen);
			break;
		case KIND_READ_PHYS:
			rc = handle_read_phys(payload, plen);
			break;
		default:
			rc = 0;
